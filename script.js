...
// Removido: window.processarOCR e window.dadosExtraidos
...
// Funções de UI mantidas (calendário, etc.)
...
function renderCalendar() { 
    ...
    if(header) { 
        header.innerHTML = `<span style="display: inline-block; vertical-align: middle;">${tituloMes}</span><button onclick="expandCalendar()" style="margin-left:10px; cursor:pointer; background: var(--navy); color: white; border: none; border-radius: 50%; width: 25px; height: 25px; font-weight: 800; font-size: 16px;">+</button>`; 
    } 
